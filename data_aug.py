...
def build_data_aug(size, mode, resnet=False, resizepad=False, device=None):
    if resnet:
...
class DataAugment(object):
    ...
    def __init__(self, opt):
        ...
        if not opt.eval:
            ...
            if self.opt.isrand_aug:
                self.augs = [
                    self.process, self.camera, self.noise, self.blur,
                    self.weather, self.pattern, self.warp, self.geometry
                ]
            # semantic augment
    ...
    def rand_aug(self, img):
        ...
        for aug in augs:
            ...
            op = aug[index]
            mag = np.random.randint(
                0, 3) if self.opt.augs_mag is None else self.opt.augs_mag
